...
- **직관적 시각화 (Interactive Visualization):** 2D/3D 디지털 트윈 기술을 도입하여 복잡한 충전소 자산 상황과 충전 트래픽을 직관적으로 관제.
- **표준 규격 준수 (Standard Compliance):** OCPP 1.6J 표준 프로토콜 완벽 지원 (Phase 2에서 OCPP 2.0.1 규격 추가 지원).

### 1.3. 시스템 단계별 개념 아키텍처 (Phased Architecture)
...
flowchart TB
    subgraph Host ["단일 호스트 (Single Host VM / Server)"]
        ...
        subgraph Admin ["관제 웹 UI"]
            VueApp["Vue 3 + Three.js Admin Console"]
        end
    end
...
### 1.4. 핵심 성능 지표 (Key Performance Indicators)
...
> **💡 Phase 1 예상 인프라 구축 비용:**
> - **시나리오 A (AWS 클라우드 임대):** m6i.2xlarge 인스턴스 (8vCPU, 32GB RAM) + EBS gp3 스토리지 구성 시 **월 약 $280 (약 370,000 KRW/월)**
> - **시나리오 B (On-Premise 물리 서버 도입):** 하드웨어 디스크 및 랜카드 물리 분리형 고성능 워크스테이션 또는 엔트리급 서버 도입 시 **초기 구매비 약 4,000,000 ~ 6,000,000 KRW (1회성, 부가세 별도)**

#### 6.2.2. Phase 2 (분산 확장 클러스터 구성 시 - 노드별 사양)
| 노드 분류 | 권장 하드웨어 사양 | 필수 탑재 소프트웨어 / 런타임 |
| :--- | :--- | :--- |
| **Application Node (x2대)** | 4 Cores CPU / 8 GB RAM / 100 GB SSD / **1 Gbps NIC** | JDK 25, Spring Boot 3.x (Web소켓 세션 맵 탑재) |
| **Kafka Broker Node (x3대)** | 4 Cores CPU / 8 GB RAM / **500 GB 이상 SSD** / **10 Gbps NIC** | Apache Kafka (KRaft Mode 실행, JVM 메모리 4G 할당, 백로그 누적 대비) |
| **PostgreSQL DB Node (x1대)** | 8 Cores CPU / 16 GB RAM / **1 TB 이상 SSD** / **10 Gbps NIC** | PostgreSQL 15/16 (OLTP 튜닝 및 WAL 로그 버퍼 조정, 복제 지연 최소화, 트랜잭션/CDR 이력용) |
| **ClickHouse DB Node (x1대)** | 16 Cores CPU / 32 GB RAM / **2 TB 이상 NVMe SSD** / **10 Gbps NIC** | ClickHouse DB (벡터화 병렬 처리 극대화를 위한 다중 코어 권장, 클러스터 동기화용, 대용량 원본 로그 적재) |

> **💡 Phase 2 예상 인프라 구축 비용 (HA 분산 확장 운영서버군):**
> - **시나리오 A (AWS 클라우드 분산 구성):** App 노드 2대, Kafka 3대, DB 2대, Redis 및 로드밸런서 전체 패키지 구성 시 **월 약 $2,630 (약 3,500,000 KRW/월)**
> - **시나리오 B (On-Premise 물리 서버 도입):** 랙 장착형 호스트 서버 2대, 16TB NAS 백업 장비, 10G 스위칭 허브 전체 구성 시 **초기 구매비 약 35,000,000 KRW (1회성, 내구연한 5년)**

...
### 2.2. Spring Integration 기반 EIP 파이프라인 설계 (Enterprise Integration Patterns)
...
* **프로세스 진화(Phase 1 ➡️ Phase 2) 전략:**
  * **Phase 1 (Standalone):** 모든 메시지 채널을 동기식 혹은 가상 스레드 기반 비동기 메모리 버스(`DirectChannel`, `ExecutorChannel`)로 구동하여 오버헤드를 극소화합니다.
  * **Phase 2 (Scale-out):** 채널 설정을 XML/Java Config 단에서 `KafkaMessageDrivenChannelAdapter` 및 `KafkaProducerMessageHandler`로 스위칭하여, 개발 코드의 변경 없이 다중 서버 분산 아키텍처로 무혈 전환합니다.

...
솔루션 내부의 모듈 구조는 하부 인프라 및 통신 프로토콜의 변화에 유연하게 대응할 수 있도록 역할별로 명확히 레이어링되어 있습니다. 아키텍처 결정 사항(제 2안 채택)에 맞춰 **[제 2안: Spring Boot 버전 - 채택]**과 **[제 1안: Spring Integration 버전 - 유보]**에 따른 세부 소프트웨어 구성의 차이를 기술합니다.

### 2.1. [제 1안: Spring Integration 버전 - 유보] EIP[^EIP] 및 메시지 채널 기반 레이어드 아키텍처
...
### 2.2. [제 2안: Spring Boot 버전 - 채택] Spring Kafka 직접 구현 기반 레이어드 아키텍처
...
| 레이어 명칭 | 제 1안 (Spring Integration 버전 - 유보) | 제 2안 (Spring Boot 버전 - 채택) |
| :--- | :--- | :--- |
...
### 2.4. [제 1안 - 유보] Spring Integration 기반 EIP 파이프라인 설계 (EIP Pipeline Detail)
...
### 2.5. [제 2안 - 채택] Spring Kafka 직접 구현 기반 파이프라인 설계
...
flowchart LR
    ...
    subgraph EAI_Engine ["메시지 라우터 & 트랜스포머 (EAI Router & Transformer - Spring Core)"]
        direction TB
        Parser["프로토콜 파서 & 유효성 검증 (Translation/Validation)"]
        Enricher["TOU[^TOU] 요금 정산 & 데이터 보강 (Enrichment/Calculation)"]
        Distributor["장애 전이/이벤트 분배기 (Routing/Distribution)"]
    ...
    CmdTopic -->|Routing to GW[^GW]| WS_Adapt
```

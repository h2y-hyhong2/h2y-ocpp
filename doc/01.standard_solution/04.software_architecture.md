...
* **구조:** ACID 트랜잭션과 데이터의 정합성이 최우선인 정보는 **PostgreSQL (OLTP[^OLTP])**에 격리 저장하고, 용량이 매우 크고 변경이 없는 계측 데이터(MeterValues) 및 원본 JSON 패킷 로그는 **ClickHouse (OLAP[^OLAP])** 시계열 테이블에 적재합니다.

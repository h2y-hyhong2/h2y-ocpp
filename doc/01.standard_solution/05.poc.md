...
- **리눅스(Linux) 환경 패키징 (Makeself 기반):**
  ...
  - **오프라인 의존성 내장:** Docker Engine 설치용 바이너리(RPM/DEB), Docker Compose CLI, 빌드된 스프링 부트 애플리케이션 JAR, MySQL 9.71/ClickHouse/Kafka의 컨테이너 이미지 아카이브(`tar`), 그리고 초기 데이터베이스 마이그레이션 스크립트 포함.
  - **설치 스크립트(`install.sh`) 기능:** OS 환경 점검(커널 버전, CPU/메모리 사양, 방화벽 포트 8080/9092/3306/8123 자동 개방), 로컬 Docker 미설치 시 내장 바이너리를 통한 자동 오프라인 설치, `docker load`를 통한 컨테이너 이미지 일괄 로드 및 `docker compose up -d`를 통한 원클릭 무인 기동.
- **윈도우(Windows) 환경 패키징 (Inno Setup 기반):**
  - Inno Setup 컴파일러를 활용하여 그래픽 UI 기반의 단일 `setup.exe` 패키지 생성.
  - **오프라인 윈도우 인프라 구성:** Windows용 WSL2 커널 패키지(MSI) 및 Docker Desktop 설치 파일을 내장하여 백그라운드로 자동 설치를 유도.
  - Windows 서비스(Service) 등록 스크립트 및 환경 변수 자동 설정을 지원하여, 설치 완료 후 재부팅 또는 기동 시 관리자 개입 없이 CSMS 포털이 자동 실행되도록 구현.
- **폐쇄망 설치 무결성 검증:**
  - 가상 머신(VM)에서 네트워크 어댑터를 완전히 차단(Host-only 또는 Internal Network)한 상태에서 설치 파일 복사 후 1-Click 설치를 실행하여, 외부 Repository(Docker Hub, Maven Central, OS Repo) 통신 없이 정상 기동 및 대시보드 진입 여부를 검증.
...
- **WebSocket & Kafka 메시지 큐 연동 및 버퍼링 검증 (Kafka Resilience & Buffering Verification):** 실시간 충전기 연결용 웹소켓 인터페이스와 Spring Integration의 `KafkaChannel`을 연동하여 메시지를 유실 없이 Kafka 브로커로 인입시키고, 트래픽 폭증 및 다운스트림 장애 상황에서 버퍼링이 정상 작동하는지 검증.
...
```mermaid
flowchart TD
    CP["가상 충전기 (Simulator)"] -->|OCPP JSON| WS

    subgraph SpringBoot ["Spring Boot POC App"]
        subgraph Producer_Zone ["Producer 영역 (Uplink & Ingress)"]
            WS["WebSocket Inbound Adapter"] -->|raw-payload| KafkaChannel["ocppRawChannel (KafkaChannel)"]
        end

        subgraph Queue_Zone ["내장/로컬 메시지 큐 (Single Kafka)"]
            Broker["Kafka Broker Engine"]
            Topic[("ocpp-raw-events <br> (Topic: Disk Queue/Storage)")]
            Broker -->|"Write (저장)"| Topic
            Topic -->|"Read (인출)"| Broker
        end

        subgraph Consumer_Zone ["Consumer 영역 (EIP Pipeline & Process)"]
            Trans["Transformer (DTO 변환)"] --> DtoCh["ocppDtoChannel (DirectChannel)"]
    ...
    %% 연결 관계 (Producer -> Queue -> Consumer)
    KafkaChannel -->|"Produce (인입)"| Broker
    Broker -->|"Consume (출구/수신)"| Trans

    %% 영역별 강조 스타일링
    style Broker fill:#ff9900,stroke:#ff5500,stroke-width:3px,color:#fff
    style Topic fill:#fff3e0,stroke:#ff9900,stroke-width:2px
    style Queue_Zone fill:#fff3e0,stroke:#ff9900,stroke-width:2px,stroke-dasharray: 5 5
    
    style SpringBoot fill:#fafafa,stroke:#37474f,stroke-width:2px
    style Producer_Zone fill:#e0f7fa,stroke:#00acc1,stroke-width:2px
    style Consumer_Zone fill:#f3e5f5,stroke:#8e24aa,stroke-width:2px
```
...
### 5.3. OCPP JSON-to-DTO[^DTO] Transformer & Router 구현
...
3. 설치 파일을 실행하여 외부 패키지 저장소 연결 시도 없이 내장된 바이너리 및 Docker 이미지만으로 1-Click 설치가 정상 완료되는지 확인합니다.
4. 설치 마법사 또는 스크립트 종료 후, 내장된 컨테이너 서비스(MySQL 9.71, Kafka, ClickHouse) 및 Spring Boot 애플리케이션이 자동으로 정상 구동 상태에 진입하는지 확인합니다.

...
```mermaid
flowchart TD
    subgraph Zone1 ["1. 충전기 단말 영역 (Edge Device)"]
        CP[("가상 충전기 Simulator <br> [단말기 Flash Storage 버퍼링]")]
    end

    subgraph Zone2 ["2. Gateway 어댑터 영역 (Ingress & Egress)"]
        direction LR
        WS_In["WS Inbound Adapter"]
        WS_Out["WS Outbound Adapter"]
    end

    subgraph Zone3 ["3. 내장/로컬 메시지 큐 (Single Kafka)"]
        direction LR
    ...
    subgraph Zone4 ["4. App Consumer & 서비스 레이어 (Spring Boot & DB)"]
        direction LR
        Trans["@KafkaListener <br> (Jackson Parser & Service Router)"]
        PG[(MySQL 9.71 OLTP)]
        CH[(ClickHouse OLAP)]

        Trans -->|Boot / Auth / Tx| PG
        Trans -->|MeterValues Batch| CH
    end

    %% 데이터 흐름 (Top-Left Charger -> Gateway -> Kafka -> Consumer/DB)
    CP -->|"1. Uplink Request"| WS_In
    WS_In -->|"2. Produce (Inbound)"| Broker
    Broker -->|"3. Consume (Inbound)"| Trans

    Trans -->|"4. Produce (Outbound)"| Broker
    Broker -->|"5. Consume (Outbound)"| WS_Out
    WS_Out -->|"6. Downlink Command"| CP

    %% 장애 처리 (Kafka Failover)
    WS_In -.->|"Kafka 장애 시 소켓 단선"| CP
    CP -.->|"단선 감지 ➡️ 로컬 버퍼링 전환"| CP

    %% 스타일링
    style Zone1 fill:#e8f5e9,stroke:#4caf50,stroke-width:1.5px
    style Zone2 fill:#e0f7fa,stroke:#00acc1,stroke-width:1.5px
    style Zone3 fill:#fff3e0,stroke:#ff9900,stroke-width:1.5px,stroke-dasharray: 4 4
    style Zone4 fill:#f3e5f5,stroke:#8e24aa,stroke-width:1.5px

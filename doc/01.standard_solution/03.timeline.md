...
* **상세 작업:**
  * 개발망 내 MySQL(OLTP[^OLTP]) 및 ClickHouse(OLAP[^OLAP] 시계열) 인스턴스 단독 노드 설치 및 초기 스키마 셋업.
...
* **상세 작업:**
  ...
  * 트랜잭션 완료 즉시 실행되는 바로정산 API 및 MySQL CDR(Charge Detail Record) 저장 로직 구현.
...
* **주요 목표:** Apache Kafka Multi-Broker 및 MySQL Primary-Standby 복제, ClickHouse 분산 레플리카 설정.
* **상세 작업:**
  * 메시지 손실 없는 분산 구성을 위한 Apache Kafka Multi-Broker 배포 및 Partitioning 설정.
  * MySQL 복제본 구성(Primary-Standby)을 통한 DB 장애 극복(Failover) 정책 적용.
...
* **상세 작업:**
  ...
  * 시스템 단일 배포용 Native 패키지 및 폐쇄망 배포용 통합 인스톨 패키지 검증 및 인도.
...
* **Back-end Engineer (2명):**
  ...
  * 가상 스레드 튜닝, MySQL & ClickHouse 하이브리드 연동, 정산 계산 엔진 및 Redis 세션 클러스터 개발.
* **Front-end Engineer (1명):**
  * Vue 3 어드민 포털 개발, 실시간 관제 UI 구현, Canvas 차고지 배치 편집기 개발.
* **DevOps / DBA (1명):**
  * MySQL 및 ClickHouse 파티셔닝, 복제 정책 셋업.

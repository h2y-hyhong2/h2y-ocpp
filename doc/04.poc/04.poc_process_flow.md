...
sequenceDiagram
    ...
    participant GW as WS Gateway (E-CNT-WS)
    participant Broker as Kafka Broker
    participant Sync as Consumer Engine (E-CNT-SYNC)
    participant Redis as Redis Cache
    participant MySQL as MySQL DB (OLTP)
    participant CH as ClickHouse DB (OLAP)
    participant Admin as 관제 API & UI (E-CTL)
...
- **1초 미터값 적재 (`MeterValues`)**:
  ...
  - 관제 백엔드(`E-CTL-API`)로 Kafka 알림 이벤트를 발행하여 **Vue 3 관제 UI 화면에 실시간 차트 반영**

...
#### 1단계: ELVIS-CONNECT 제품군 개발 (2026-08-17 ~ 2026-09-23)
...
- **3~4주차 (2026-08-28 ~ 2026-09-11):**
  - **[elvis-connect-ws]** Kafka `ocpp-raw-events` 인바운드 Producer 및 `ocpp-outbound-commands` 중계 개발 (`WBS-CONNECT-WS-104-C` ~ `107-C`).
  - **[elvis-connect-sync]** Virtual Threads 기반 Consumer 스캐폴딩 구축 및 OCPP 메시지 DTO 변환/파서 구현 (`WBS-CONNECT-SYNC-101-A` ~ `107-C`).
- **5~6주차 (2026-09-14 ~ 2026-09-23):**
  - **[elvis-connect-sync]** 계절별/시간대별 단가(TOU) 계산 연산 엔진, 과금 CDR 원장 DB 저장 및 ClickHouse 1초 미터값 벌크 적재 엔진 구현 (`WBS-CONNECT-SYNC-105-C` ~ `110-C`).
  - **★ Milestone 1 (2026-09-23)**: 추석 연휴 전 1초 적재 & TOU CDR 파이프라인 단독 완수 마감.

#### [추석 연휴 및 대체공휴일 휴무] (2026-09-24 ~ 2026-09-28, 5일간 휴무)

#### 2단계: ELVIS-CONTROL 제품군 개발 (2026-09-29 ~ 2026-12-04)

- **7~9주차 (2026-09-29 ~ 2026-10-16):**
  - **[elvis-control-api]** 관제 어드민 REST API 요구사항 분석, 레이어 아키텍처 설계, 자산 마스터 페이징 API 및 실시간 상태 조회 API 구현 (`WBS-CONTROL-API-101-A` ~ `102-C`).
- **10~12주차 (2026-10-19 ~ 2026-11-04):**
  - **[elvis-control-api]** 충전 거래 이력 API, ClickHouse 시계열 통계/원본로그 API, Kafka 하향 제어 Producer 및 어드민 JWT 인증 API 구현 (`WBS-CONTROL-API-103-C` ~ `109-C`).
- **13~16주차 (2026-11-05 ~ 2026-12-04):**
  - **[elvis-control-ui]** Vue 3 + Vite 스캐폴딩, 관제 대시보드 상태 카드, 실시간 전력/단가 차트, GIS 충전소 지도 맵, 충전기 그리드/모달, 원격제어 팝업, 통신로그 뷰어, TOU 설정 UI 및 Nginx 서빙 구현 (`WBS-CONTROL-UI-101-C` ~ `109-C`).
...
* **`ELVIS-CONTROL`**: 관제 포털 및 백엔드 관리 어드민 제품군
  ...
  * **`ELVIS-CONTROL-API`** (`elvis-control-api`): 관제 백엔드 REST API 서비스
* **`ELVIS-CONNECT`**: 충전기 단말 접속 및 데이터 파이프라인 제품군
  * **`ELVIS-CONNECT-WS`** (`elvis-connect-ws`): OCPP 웹소켓 게이트웨이
  * **`ELVIS-CONNECT-SYNC`** (`elvis-connect-sync`): 패킷 파싱 및 DB 이중 적재 Consumer 백엔드

#### 4대 소프트웨어 모듈 명세표

| 상위 제품군 | 모듈명 (Module Name) | 대문자 풀명칭 (Code) | 주요 기술 스택 | 매핑 영역 | 핵심 역할 및 기능 범위 |
| :--- | :--- | :---: | :--- | :---: | :--- |
| **ELVIS-CONTROL** | **`elvis-control-ui`** <br>(관제 UI 프론트엔드) | **`ELVIS-CONTROL-UI`** | Vue 3 / SPA <br>(Nginx / Vite Reverse Proxy) | `Zone 1` | • 관제사 어드민 웹 UI 포털 및 실시간 대시보드 시각화 <br>• Nginx/Vite 내장 프록시로 `/api` 요청을 관제 백엔드로 포워딩 |
| **ELVIS-CONTROL** | **`elvis-control-api`** <br>(관제 백엔드 API 서비스) | **`ELVIS-CONTROL-API`** | Java 25 / Spring Boot <br>(Spring MVC, WebSocket Handler) | `Zone 5` | • 어드민 REST API (MySQL 9.71 & ClickHouse 이중 직접 조회) <br>• UI 웹소켓 푸시 핸들러 내장 (`ocpp-ui-notifications` 소비 ➔ UI Push) <br>• 관제사 원격 제어 명령 Kafka 발행 (`ocpp-outbound-commands`) |
...
| **ELVIS-CONNECT** | **`elvis-connect-sync`** <br>(OCPP Consumer 백엔드) | **`ELVIS-CONNECT-SYNC`** | Java 25 / Spring Boot <br>(Virtual Threads, KafkaListener) | `Zone 6` | • Kafka 인바운드 메시지 소비 (`ocpp-raw-events`) <br>• OCPP 패킷 파싱, MySQL 9.71 상태 Update, ClickHouse 벌크 적재 <br>• 장애/상태 변경 발생 시 Kafka UI 알림 토픽으로 이벤트 발행 |

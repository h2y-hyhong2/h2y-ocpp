...
| **01** | **PoC 아키텍처 명세서** | [01.poc_architecture.md](file:///d:/project/lselink/ocpp-lite/git/h2y-ocpp/doc/04.poc/01.poc_architecture.md) | **[본 문서]** 2대 제품군, 4대 독립 모듈, CQRS 하이브리드 DB 및 Kafka 버퍼링 아키텍처 설계서 |
| **02** | **PoC WBS 일정 명세서** | [02.poc_feature_matrix.md](file:///d:/project/lselink/ocpp-lite/git/h2y-ocpp/doc/04.poc/02.poc_feature_matrix.md) | 100% 제품군 직렬화, 주말/공휴일 휴무 적용, 12/04 확충 및 YYYY-MM-DD 타깃 일정 명세서 |
| **03** | **PoC 통합 세부 과제 명세서** | [03.poc_task_list.md](file:///d:/project/lselink/ocpp-lite/git/h2y-ocpp/doc/04.poc/03.poc_task_list.md) | 제품군/모듈이 좌측에 명시된 전수 68개 WBS 세부 과제 통합 레지스트리 명세서 |
| **04** | **PoC 법인 충전 프로세스 명세서**| [04.poc_process_flow.md](file:///d:/project/lselink/ocpp-lite/git/h2y-ocpp/doc/04.poc/04.poc_process_flow.md) | PoC 수검/시연 전용 법인 회원(UserType.C) RFID 카드 태깅 충전 E2E 프로세스 & TOU 정산 명세서 |

---

## 1. POC 6개월 수행 로드맵 (6-Month POC Roadmap)

본 기술 검증(POC)은 Phase 1 Standalone 모드 구축 일정과 싱크를 맞추어 총 6개월 일정(2026.07.01 ~ 2026.12.31)으로 진행되며, **분석(1개월) -> 설계(1개월) -> 개발(3.2개월) -> 완료(1개월)**의 4단계 추진 체계로 세분화하여 수행됩니다.

```mermaid
gantt
    title Phase 1 기술 검증(POC) 4단계 수행 일정 (2026.07.01 ~ 2026.12.31)
    dateFormat  YYYY-MM-DD
    axisFormat  %Y-%m-%d

    section 1. 분석
    요건 분석 및 기능 정의 :active, poc_analysis, 2026-07-01, 2026-08-01

    section 2. 설계
    시스템 아키텍처 및 상세 설계 :poc_design, 2026-08-02, 2026-08-29

    section 3. 개발
    개발환경 구축            :poc_dev1, 2026-08-30, 2026-09-26
    OCPP 프로토콜 연동        :poc_dev2, 2026-09-27, 2026-10-24
    관제 UI 및 부하 테스트    :poc_dev3, 2026-10-25, 2026-12-05

    section 4. 완료
    통합 시나리오 검증 및 배포 패키징 마감 :poc_final, 2026-12-06, 2026-12-31
```

### 1.1. 주차별 세부 마일스톤 및 수행 작업 (Weekly Detailed Milestones & Date Ranges) 2026.07.01 ~ 2026.12.31

#### 1단계: 분석 - 요건 분석 및 기능 정의 (2026.07.01 ~ 2026.08.01)

- **1주차 (Week 1 / 2026.07.01수 ~ 2026.07.11토):**
...
- **2주차 (Week 2 / 2026.07.12일 ~ 2026.07.18토):**
...
- **3주차 (Week 3 / 2026.07.19일 ~ 2026.07.25토):**
...
- **4주차 (Week 4 / 2026.07.26일 ~ 2026.08.01토):**
...
#### 2단계: 설계 - 시스템 아키텍처 및 상세 설계 (2026.08.02 ~ 2026.08.29)

- **5주차 (Week 5 / 2026.08.02일 ~ 2026.08.08토):**
...
- **6주차 (Week 6 / 2026.08.09일 ~ 2026.08.15토):**
...
- **7주차 (Week 7 / 2026.08.16일 ~ 2026.08.22토):**
...
- **8주차 (Week 8 / 2026.08.23일 ~ 2026.08.29토):**
...
#### 3단계: 개발 - 개발환경 구축, OCPP 프로토콜 연동 및 관제 구현 (2026.08.30 ~ 2026.12.05)

##### 개발환경 구축 및 인프라 연동 (2026.08.30 ~ 2026.09.26)

- **9주차 (Week 9 / 2026.08.30일 ~ 2026.09.05토):**
...
- **10주차 (Week 10 / 2026.09.06일 ~ 2026.09.12토):**
...
- **11주차 (Week 11 / 2026.09.13일 ~ 2026.09.19토):**
  - **[웹소켓 구축]** Spring WebSocket Endpoint 핸들러 구현 및 `ConcurrentHashMap` 기반 메모리 세션 관리 클래스 `LocalSessionStore` 구축.
  - **[메시지 채널]** Spring WebSocket 핸들러에서 `KafkaTemplate`을 사용해 웹소켓 유입 Raw JSON 패킷을 Kafka `ocpp-raw-events` 토픽으로 유실 없이 퍼블리싱하는 비동기 파이프라인 검증.
- **12주차 (Week 12 / 2026.09.20일 ~ 2026.09.26토):**
  - **[환경 검증]** 개발 환경 인프라(MySQL, ClickHouse, Kafka, WebSocket GW) 연동 상태 점검 및 초기 통합 빌드 파이프라인 정립.

##### OCPP 프로토콜 연동 및 비즈니스 백엔드 개발 (2026.09.27 ~ 2026.10.24)

- **13주차 (Week 13 / 2026.09.27일 ~ 2026.10.03토):**
...
- **14주차 (Week 14 / 2026.10.04일 ~ 2026.10.10토):**
...
- **15주차 (Week 15 / 2026.10.11일 ~ 2026.10.17토):**
  - **[비즈니스 API]** 법인/사업장 마스터, 충전소 목록, 충전기 자산 조회, 한전 요금 테이블 연동 REST API 개발.
  - **[인증/보안]** 충전기 웹소켓 접속 시 Basic Auth HTTP 헤더 인증 처리 필터 및 스프링 시큐리티 기본 적용.
- **16주차 (Week 16 / 2026.10.18일 ~ 2026.10.24토):**
...
##### 관제 UI 개발, 정산 연동 및 부하 테스트 (2026.10.25 ~ 2026.12.05)

- **17주차 (Week 17 / 2026.10.25일 ~ 2026.10.31토):**
  - **[FE 기본 레이아웃]** Vue 3 (Vite, TypeScript) 개발 환경 세팅, Pinia 상태 관리 아키텍처 수립 및 어드민 기본 포털 레이아웃(Sidebar, Header, Main) 구성.
  - **[FE 관제 대시보드]** 메인 관제 대시보드 컴포넌트(실시간 기동률, 충전소 상태 카드 그리드) 구현.
- **18주차 (Week 18 / 2026.11.01일 ~ 2026.11.07토):**
...
- **19주차 (Week 19 / 2026.11.08일 ~ 2026.11.14토):**
  - **[장애 관제]** OCPP `StatusNotification` Faulted 수신 시 MySQL 9.71 장애 이력 테이블 생성 및 `alertPushChannel`을 통한 실시간 장애 알림 팝업 연동.
  - **[원격 관제/제어]** 어드민 API 원격 제어 명령(`Reset`, `UnlockConnector`) 수신 시 active WebSocket 세션으로 OCPP Call 패킷 즉시 라우팅 및 UI 제어 연동.
- **20주차 (Week 20 / 2026.11.15일 ~ 2026.11.21토):**
  - **[관제 UI 통합]** 실시간 OCPP 통신 로그 뷰어 화면, 특정 기기 트래픽 격리(Sandbox) 및 패킷 디버깅(WireTap) 스위칭 관리자 콘솔 UI 통합 검증.
- **21주차 (Week 21 / 2026.11.22일 ~ 2026.11.28토):**
...
- **22주차 (Week 22 / 2026.11.29일 ~ 2026.12.05토):**
...
#### 4단계: 완료 - 통합 검증, 배포 패키징 및 최종 마감 (2026.12.06 ~ 2026.12.31)

- **23주차 (Week 23 / 2026.12.06일 ~ 2026.12.12토):**
...
- **24주차 (Week 24 / 2026.12.13일 ~ 2026.12.19토):**
  - **[인프라/배포]** 완전 폐쇄망 환경의 가상 머신에 인스톨러 배포 후 1-Click 설치 기동 완료 및 패키징 수검.
- **25주차 (Week 25 / 2026.12.20일 ~ 2026.12.26토):**
  - **[마일스톤 검증]** M1 마일스톤(Phase 1 POC 통과) 최종 확인 및 시스템 산출물(ADD/IDD/FDS) 최종본 동기화.
- **26주차 (Week 26 / 2026.12.27일 ~ 2026.12.31목):**
  - **[최종 마감]** Phase 1 기술 검증(POC) 최종 결과 보고서 작성, 프로젝트 인수도서 전달 및 POC 수행 완료.
...
### 3.1. 4대 핵심 소프트웨어 모듈 및 2대 제품군 체계 (Product Suite & Subsystem Architecture)

인프라(Kafka, DB) 및 엣지 단말(Simulator)을 제외한 실제 소프트웨어 개발 및 배포 대상은 **2대 핵심 제품군 (`ELVIS-CONTROL`, `ELVIS-CONNECT`)** 산하 **4개 독립 모듈**로 구성되며, 직관적인 식별 및 운영을 위한 모듈 약어(Abbreviation) 체계는 다음과 같습니다:

#### 📌 제품군 및 모듈 약어 체계 (Product Suite & Abbreviations)
* **`ELVIS-CONTROL` (`E-CTL`)**: 관제 포털 및 백엔드 관리 어드민 제품군
  * **`E-CTL-UI`** (`elvis-control-ui`): 관제 UI 프론트엔드
  * **`E-CTL-API`** (`elvis-control-api`): 관제 백엔드 REST API 서비스
* **`ELVIS-CONNECT` (`E-CNT`)**: 충전기 단말 접속 및 데이터 파이프라인 제품군
  * **`E-CNT-WS`** (`elvis-connect-ws`): OCPP 웹소켓 게이트웨이
  * **`E-CNT-SYNC`** (`elvis-connect-sync`): 패킷 파싱 및 DB 이중 적재 Consumer 백엔드

#### 4대 소프트웨어 모듈 명세표

| 상위 제품군 | 모듈명 (Module Name) | 약어 (Code) | 주요 기술 스택 | 매핑 영역 | 핵심 역할 및 기능 범위 |
| :--- | :--- | :---: | :--- | :---: | :--- |
| **ELVIS-CONTROL** | **`elvis-control-ui`** <br>(관제 UI 프론트엔드) | **`E-CTL-UI`** | Vue 3 / SPA <br>(Nginx / Vite Reverse Proxy) | `Zone 1` | • 관제사 어드민 웹 UI 포털 및 실시간 대시보드 시각화 <br>• Nginx/Vite 내장 프록시로 `/api` 요청을 관제 백엔드로 포워딩 |
| **ELVIS-CONTROL** | **`elvis-control-api`** <br>(관제 백엔드 API 서비스) | **`E-CTL-API`** | Java 25 / Spring Boot <br>(Spring MVC, WebSocket Handler) | `Zone 5` | • 어드민 REST API (MySQL 9.71 & ClickHouse 이중 직접 조회) <br>• UI 웹소켓 푸시 핸들러 내장 (`ocpp-ui-notifications` 소비 ➔ UI Push) <br>• 관제사 원격 제어 명령 Kafka 발행 (`ocpp-outbound-commands`) |
| **ELVIS-CONNECT** | **`elvis-connect-ws`** <br>(OCPP WS 게이트웨이) | **`E-CNT-WS`** | Java 25 / Spring Boot <br>(Spring WebSocket) | `Zone 3` | • 충전기 단말 WebSocket 커넥션 수용 (Handshake/Auth) <br>• 수신 패킷 Kafka 인바운드 발행 및 하향 제어 커맨드 수신/전송 |
| **ELVIS-CONNECT** | **`elvis-connect-sync`** <br>(OCPP Consumer 백엔드) | **`E-CNT-SYNC`** | Java 25 / Spring Boot <br>(Virtual Threads, KafkaListener) | `Zone 6` | • Kafka 인바운드 메시지 소비 (`ocpp-raw-events`) <br>• OCPP 패킷 파싱, MySQL 9.71 상태 Update, ClickHouse 벌크 적재 <br>• 장애/상태 변경 발생 시 Kafka UI 알림 토픽으로 이벤트 발행 |

---

## 4. 단일 서버 일체형 환경 구성 및 사양 명세 (Single Server All-in-One Host Environment)

본 POC 및 Phase 1 인스톨러 배포본은 단일 PC 또는 단일 VM 호스트(**Windows 11 64-bit** 또는 Enterprise Linux) 환경에 **4대 앱 모듈**과 **3대 인프라 미들웨어(Kafka, MySQL, ClickHouse)**를 모두 함께 탑재(All-in-One Deployment)하여 운영할 수 있도록 검증 사양을 제시합니다.
...
| :----------------------------------------- | :-------------------------- | :---------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------- |
| **Disk 1 (`C:\`)** <br>(System & OLTP)     | **400 GB NVMe SSD**         | **650 GB NVMe SSD**           | • **Windows 11 OS** (`C:\Windows`, Pagefile) <br>• **4대 앱 모듈** (`C:\ocpp\apps`, 앱 로그) <br>• **MySQL 9.71 OLTP DB** (`C:\ProgramData\MySQL`) | • **랜덤 I/O 위주 업무 처리** <br>• OS 부팅, 웹 앱 서비스, MySQL 회원/원장 SQL 트랜잭션 수용              |

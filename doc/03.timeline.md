...
본 문서는 CPO 통합 운영 솔루션(CSMS Platform) 구축을 위한 단계별 개발 로드맵 및 상세 개발 일정을 기술합니다. 본 프로젝트는 **Phase 1 (단일 서버 기동 모드 - 필수 기능)**의 안정성을 극대화하기 위해 **초기 6개월(24주)** 동안의 설계, 구현, 튜닝 및 통합 검증을 진행하며, 대규모 분산 스케일 아웃 및 대외 연동 요건이 포함된 **Phase 2 (분산 확장 클러스터 및 고도화 기능)**에 대해서도 **6개월(24주)의 넉넉한 안정화 개발 기간**을 부여하여 총 12개월(1년)의 장기 안정 로드맵으로 구성합니다.

※ **3D 디지털 트윈 관제 기능**은 Phase 1 필수 요건에서 제외되었으며, 전체 일정에 영향을 주지 않도록 **별도 추가 트랙(Optional)**으로 분리하여 관리합니다.
...
| :--- | :--- | :--- | :--- |
| **M1: Phase 1 완료** | 단일 호스트 기동 모드 구현 | 통합 기능 정의서 상의 Phase 1 필수 기능(OCPP 1.6J 연동, 일반 관제 대시보드, 정산 원장 저장 및 단독 튜닝)의 100% 완료 및 안정화 검증 (3D 관제 제외) | **24주차 말 (6개월차)** |
...
gantt
    ...
    section 별도 추가 트랙 (Optional)
    3D 디지털 트윈 관제 개발 (별도 일정) : p3_twin, after p2_final, 28d
```
...
* **주요 목표:** TOU 요금 계산 정산 엔진 개발 및 기기 장애 접수 처리 프로세스 구축.
* **상세 작업:**
...
* **상세 작업:**
  ...
  * 전기 버스 모델 관리, 차량 및 idTag 정보 매핑 및 배터리 SOH(State of Health) 모니터링 팝업 구축.
  * 노선 버스 운수사 전용 충전 요율 및 요금 정산 리포트 모듈 개발.

### 2.10. [Phase 2 - 10개월차] 스마트 차징 및 원격 FOTA 관리 고도화 (Week 37 - 40)
...
* **상세 작업:**
  ...
  * OCPP 보안 규격을 위한 TLS 암호화 키 관리 및 기기 개별 인증서 발급 프로세스 적용.

### 2.11. [Phase 2 - 11개월차] 대외 로밍 중계 연동 및 결제 취소 수동 관리 (Week 41 - 44)
* **주요 목표:** 환경부/한전 로밍 정보 연동 및 PG 결제 취소 건 예외 복구 관리자 화면 구현.
* **상세 작업:**
...
* **상세 작업:**
  ...
  * 가상 시뮬레이터 활용 동시 2,000 TPS 대용량 부하 성능 테스트 검증 완료.
  * 쿠버네티스 Helm Chart 및 폐쇄망 배포용 통합 Docker 패키지 검증 및 인도.

---

### 2.13. [별도 추가 트랙] 3D 디지털 트윈 관제 고도화 (Optional Track - 별도 일정)
* **주요 목표:** Three.js WebGL 기반의 충전소 입체 디지털 트윈 관제 화면 구축 (필수 일정에서 격리).
* **상세 작업:**
  * Three.js WebGL 기반의 충전소 부지/건물 평면도 2.5D/3D Mesh 렌더링 및 로딩 속도 최적화.
  * 실시간 충전기 동작 상태(Online/Offline/Faulted/Charging) 동적 Glow Effect 시각화 적용.
  * 마우스 오버 툴팁 오버레이를 활용한 개별 충전기 실시간 계측 요약 팝업 연동.
...
* **Front-end Engineer (1명):**
  ...
  * (3D 추가 트랙 진행 시) Three.js WebGL 3D/2.5D 충전소 모델 렌더링 및 모션 구현.
* **DevOps / DBA (1명):**

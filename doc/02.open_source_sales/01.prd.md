# [PRD] CPO[^CPO] CSMS[^CSMS] Lite (Integrated CSMS Platform)
...
본 프로젝트는 수주 영업(Loss Leader) 및 조기 시장 선점을 목적으로 하여, Phase 1 (Community Edition 영업 릴리스) 6개월, Phase 2 (본사 통합 허브 및 HA[^HA] 클러스터 고도화) 6개월을 배정하여 총 12개월(1년)의 일정으로 진행됩니다.
...
| :--- | :--- | :--- | :--- |
| **M1: Phase 1 완료** | 영업 수주용 1-Click 패키지 및 화이트라벨링 릴리스 | 영업부서 제안용 Community Edition(Docker-Compose) 배포 패키지 완성, 위탁 파트너사 로고/테마 맞춤화(White-labeling) 엔진 최우선 구현, 현장 엔지니어 자가 진단 UI[^UI] 및 MySQL 싱글 DB[^DB] 기반 초경량 구동 검증 완료 | **24주차 말 (6개월차)** |
| **M2: Phase 2 개발 완료** | 본사 통합 허브 및 데이터 동기화 파이프라인 개발 | 각 영업지(CE[^CE] Edge)로부터 충전 원장(CDR) 및 시계열 미터링 요약을 본사 서버로 이관/수집하는 Edge-to-Cloud 동기화 API[^API] 파이프라인 완성, Redis 분산 세션 및 다중 노드 제어 명령 라우팅 연동 완료 | **40주차 말 (10개월차)** |
...
단일 가상 머신(VM) 또는 하드웨어 내에서 Kafka 단일 브로커(KRaft 모드)와 MySQL 단독 인스턴스만을 결합하여 구동합니다. 웹소켓 게이트웨이와 비즈니스 코어 간의 주요 채널(ocppRawChannel, controlCommandChannel)에 Kafka 단일 브로커를 메시지 버스로 연동하여 실시간 완충 및 장애 격리를 활성화하며, 별도의 분산 세션 공유 미들웨어(Redis) 없이 JVM[^JVM] 로컬 메모리로 세션을 관리합니다.
...
flowchart TB
    ...
    subgraph DB_Cluster ["분산 퍼시스턴스 레이어"]
        MySQLPrimary["MySQL Primary (OLTP Write)"]
        MySQLStandby["MySQL Standby (OLTP Read)"]
...
단일 가상 머신(VM) 또는 하드웨어 내에서 Spring Integration EIP 추상화 없이, 단일 JVM 호스트 내에 Spring-Kafka 모듈을 직접 연동하여 단일 Kafka 브로커와 데이터베이스(MySQL/ClickHouse)를 통해 구동하는 단독 배포 구성입니다.
...
* **핵심 구조 및 설계 원칙:**
  ...
  * **CQRS[^CQRS] 데이터 이원화 저장:** 정적 데이터 변경 및 금융/정산 정합성을 보장하는 정보는 MySQL(OLTP)이 처리하고, 수천 대의 충전기로부터 쏟아지는 변경 불가능한 실시간 패킷 및 미터값 로그는 ClickHouse(OLAP) 시계열 테이블에 고속으로 벌크 인서트 처리합니다.
...
- **메시지 처리 성능 (Throughput):** Peak 시 최소 2,000 TPS[^TPS](Transactions Per Second) 이상의 OCPP 메시지 처리.
...
## 5. UI/UX[^UX] 디자인 및 화면 구성 요구사항 (UI/UX Guide)
...
| **Memory (RAM[^RAM])** | 32 GB RAM | 64 GB RAM 이상 | Spring Boot (4GB), Kafka (2GB), MySQL (4GB), ClickHouse (8GB), 여유 OS 페이지 캐시 영역 필수. |
...
| **MySQL DB Node (x1대)** | 4 Cores CPU / 8 GB RAM / 300 GB SSD / 1 Gbps NIC | 8 Cores CPU / 16 GB RAM / 1 TB 이상 SSD / 10 Gbps NIC | MySQL 8.0+ (OLTP 튜닝 및 Redo 로그 버퍼 조정, 복제 지연 최소화, 트랜잭션/CDR 이력용) |
...
[^PG[^PG]]: **PG (Payment Gateway):** 전자 결제 대행사
...
[^WAL[^WAL]]: **WAL (Write-Ahead Logging):** 로그 선행 기입 (데이터베이스의 트랜잭션 안정성 확보 로그)

---
[^PG]: **PG (Payment Gateway):** 전자 결제 대행사
[^WAL]: **WAL (Write-Ahead Logging):** 로그 선행 기입 (데이터베이스의 트랜잭션 안정성 확보 로그)
